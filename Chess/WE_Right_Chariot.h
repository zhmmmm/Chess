...
public:
	...
	void PieceLogicUpdata(int Which, int State, int Mouse_X, int Mouse_Y)  //棋子逻辑更新
	{
		if (Piece::Reutrn_mWe() == 0)//0表示我可以走棋了
		{
			if (Which == 1 && State == 1)
			{
				if (Mouse_X >= mWE_Right_Chariot_X - 30 && Mouse_X <= mWE_Right_Chariot_X + 30
					&& Mouse_Y >= mWE_Right_Chariot_Y - 30 && Mouse_Y <= mWE_Right_Chariot_Y + 30)
				{
					if (Piece_Logic::mWE_CommandState || Piece_Logic::mWE_CommandState == 0)
					{
						Piece_Logic::mWE_CommandState = 1;//右边的車被锁定
					}
				}
				if (Piece_Logic::mWE_CommandState == 1)//右边的車被锁定
				{
					BOOL BoolForward = FALSE;
					BoolForward = _____Elephant__TO__Soldier__Logic__::Chariot_MoveForwardTarget(mWE_Right_Chariot_X, mWE_Right_Chariot_Y, Mouse_X, Mouse_Y);
					BOOL BoolBack = FALSE;
					BoolBack = _____Elephant__TO__Soldier__Logic__::Chariot_MoveBackTarget(mWE_Right_Chariot_X, mWE_Right_Chariot_Y, Mouse_X, Mouse_Y);
					BOOL BoolLeft = FALSE;
					BoolLeft = _____Elephant__TO__Soldier__Logic__::Chariot_MoveLeftTarget(mWE_Right_Chariot_X, mWE_Right_Chariot_Y, Mouse_X, Mouse_Y);
					BOOL BoolRight = FALSE;
					BoolRight = _____Elephant__TO__Soldier__Logic__::Chariot_MoveRightTarget(mWE_Right_Chariot_X, mWE_Right_Chariot_Y, Mouse_X, Mouse_Y);
					int TempVar = __WE_Right_Chariot___(BoolForward, BoolBack, BoolLeft, BoolRight);
				}
			}
		}
	}
	int __WE_Right_Chariot___(BOOL BoolForward, BOOL BoolBack, BOOL BoolLeft, BOOL BoolRight)
	{
		if (BoolForward)
		{
			for (int i = 0; i < 12; i++)
			...
			for (int i = 0; i < 12; i++)
			{
				...
				for (int j = 0; j < 11; j++)
				{
					...
					if (Piece_Logic::mMouseDownRange[i][j] == 1)//索引到棋子左边的車
					{
						...
						int Move_Y = (Temp_Y - mWE_Right_Chariot_Y) / 60;//得到要移动的次数，同时也是目标的Y坐标
			...
			for (int i = 0; i < 12; i++)
			{
				...
				for (int j = 0; j < 11; j++)
				{
					...
					if (Piece_Logic::mMouseDownRange[i][j] == 1)//索引到棋子左边的車
					{
						...
						int Move_X = (mWE_Right_Chariot_X - Temp_X) / 60;//得到要移动的次数，同时也是目标的Y坐标
			...
			for (int i = 0; i < 12; i++)
			{
				...
				for (int j = 0; j < 11; j++)
				{
					...
					if (Piece_Logic::mMouseDownRange[i][j] == 1)//索引到棋子左边的車
					{
						...
						int Move_X = (Temp_X - mWE_Right_Chariot_X) / 60;//得到要移动的次数，同时也是目标的Y坐标
