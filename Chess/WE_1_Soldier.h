...
//小兵 1 
class WE_1_Soldier :public Piece, public Piece_Logic,public _____Elephant__TO__Soldier__Logic__
{
	int mWE_1_Soldier_Bmpid = 0;

	int mWE_1_Soldier_X = 0;
	int mWE_1_Soldier_Y = 0;
...
public:
	...
	~WE_1_Soldier()
	{
		GoBitmap->ReleaseBmp(mWE_1_Soldier_Bmpid);
	}
	void InitPiece()                    //加载棋子
	{
		mWE_1_Soldier_Bmpid = GoBitmap->Load_Bmp("Image\\Piece\\Blue_Square\\Soldier.bmp");
	}
	void RenderPiece()            //渲染棋子
	{
		m.Translate(mWE_1_Soldier_X, mWE_1_Soldier_Y);
		GoBitmap->SetWorldTransform(m);
		GoBitmap->DrawBmp2(mWE_1_Soldier_Bmpid, -30, -30, 60, 60, 0, 0, 60, 60, 0XFF00FF);
		m.Identity();
		GoBitmap->SetWorldTransform(m);
	}
	void PieceLogicUpdata(int Which, int State, int Mouse_X, int Mouse_Y)  //棋子逻辑更新
	{
		if (Piece::Reutrn_mWe() == 0)//0表示我可以走棋了
		{
			if (Which == 1 && State == 1)
			{
				if (Mouse_X >= mWE_1_Soldier_X - 30 && Mouse_X <= mWE_1_Soldier_X + 30
					&& Mouse_Y >= mWE_1_Soldier_Y - 30 && Mouse_Y <= mWE_1_Soldier_Y + 30)
				{
					if (Piece_Logic::mWE_CommandState || Piece_Logic::mWE_CommandState == 0)
					{
						Piece_Logic::mWE_CommandState = 16;//小兵被锁定
					}
				}
				if (Piece_Logic::mWE_CommandState == 16)//小兵被锁定
				{
					BOOL BoolForward = FALSE;
					BOOL BoolLeft = FALSE;
					BOOL BoolRight = FALSE;
					BoolForward = _____Elephant__TO__Soldier__Logic__::Soldier_MoveForwardTarget(mWE_1_Soldier_X, mWE_1_Soldier_Y, Mouse_X, Mouse_Y);
					BoolLeft = _____Elephant__TO__Soldier__Logic__::Soldier_MoveLeftTarget(mWE_1_Soldier_X, mWE_1_Soldier_Y, Mouse_X, Mouse_Y);
					BoolRight = _____Elephant__TO__Soldier__Logic__::Soldier_MoveRightTarget(mWE_1_Soldier_X, mWE_1_Soldier_Y, Mouse_X, Mouse_Y);
	...
	{
		__BoolForward(BoolForward, mWE_1_Soldier_Y, 16, 1);
		__BoolLeft(BoolLeft, mWE_1_Soldier_X, 16, 1);
		__BoolRight(BoolRight, mWE_1_Soldier_X, 16, 1);
		return 0;
	}
