...
//小兵 3
class WE_3_Soldier :public Piece, public Piece_Logic,public _____Elephant__TO__Soldier__Logic__
{
...
public:
	...
	void PieceLogicUpdata(int Which, int State, int Mouse_X, int Mouse_Y)  //棋子逻辑更新
	{
		if (Piece::Reutrn_mWe() == 0)//0表示我可以走棋了
		{
			if (Which == 1 && State == 1)
			{
				if (Mouse_X >= mWE_3_Soldier_X - 30 && Mouse_X <= mWE_3_Soldier_X + 30
					&& Mouse_Y >= mWE_3_Soldier_Y - 30 && Mouse_Y <= mWE_3_Soldier_Y + 30)
				{
					if (Piece_Logic::mWE_CommandState || Piece_Logic::mWE_CommandState == 0)
					{
						Piece_Logic::mWE_CommandState = 14;//小兵被锁定
					}
				}
				if (Piece_Logic::mWE_CommandState == 14)//小兵被锁定
				{
					BOOL BoolForward = FALSE;
					BOOL BoolLeft = FALSE;
					BOOL BoolRight = FALSE;
					BoolForward = _____Elephant__TO__Soldier__Logic__::Soldier_MoveForwardTarget(mWE_3_Soldier_X, mWE_3_Soldier_Y, Mouse_X, Mouse_Y);
					BoolLeft = _____Elephant__TO__Soldier__Logic__::Soldier_MoveLeftTarget(mWE_3_Soldier_X, mWE_3_Soldier_Y, Mouse_X, Mouse_Y);
					BoolRight = _____Elephant__TO__Soldier__Logic__::Soldier_MoveRightTarget(mWE_3_Soldier_X, mWE_3_Soldier_Y, Mouse_X, Mouse_Y);
