...
class ENEMY_1_Soldier :public Piece, public Piece_Logic, public _____Elephant__TO__Soldier__Logic__
{
	int mENEMY_1_Soldier_Bmpid = 0;
	int mENEMY_1_Soldier_X;
	int mENEMY_1_Soldier_Y;
...
public:
	...
	void PieceLogicUpdata(int Which, int State, int Mouse_X, int Mouse_Y)  //棋子逻辑更新
	{
		if (Piece::Reutrn_mWe() == 1)//1表示我可以走棋了
		{
			if (Which == 1 && State == 1)
			{
				if (Mouse_X >= mENEMY_1_Soldier_X - 30 && Mouse_X <= mENEMY_1_Soldier_X + 30
					&& Mouse_Y >= mENEMY_1_Soldier_Y - 30 && Mouse_Y <= mENEMY_1_Soldier_Y + 30)
				{
					if (Piece_Logic::mWE_CommandState || Piece_Logic::mWE_CommandState == 0)
					{
						Piece_Logic::mWE_CommandState = 160;//小兵被锁定
					}
				}
				if (Piece_Logic::mWE_CommandState == 160)//小兵被锁定
				{
					BOOL BoolForward = FALSE;
					BOOL BoolLeft = FALSE;
					BOOL BoolRight = FALSE;
					BoolForward = _____Elephant__TO__Soldier__Logic__::ENEMY__Soldier_MoveForwardTarget(mENEMY_1_Soldier_X, mENEMY_1_Soldier_Y, Mouse_X, Mouse_Y);
					BoolLeft = _____Elephant__TO__Soldier__Logic__::ENEMY__Soldier_MoveLeftTarget(mENEMY_1_Soldier_X, mENEMY_1_Soldier_Y, Mouse_X, Mouse_Y);
					BoolRight = _____Elephant__TO__Soldier__Logic__::ENEMY__Soldier_MoveRightTarget(mENEMY_1_Soldier_X, mENEMY_1_Soldier_Y, Mouse_X, Mouse_Y);
	...
	{
		__BoolForward(BoolForward, mENEMY_1_Soldier_Y, 160, 0);
		__BoolLeft(BoolLeft, mENEMY_1_Soldier_X, 160, 0);
		__BoolRight(BoolRight, mENEMY_1_Soldier_X, 160, 0);
		return 0;
	}
