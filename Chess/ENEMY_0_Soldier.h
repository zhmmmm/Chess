...
class ENEMY_0_Soldier :public Piece, public Piece_Logic,public _____Elephant__TO__Soldier__Logic__
{
	int mENEMY_0_Soldier_Bmpid = 0;
	int mENEMY_0_Soldier_X;
	int mENEMY_0_Soldier_Y;
...
public:
	ENEMY_0_Soldier()
	{
		mENEMY_0_Soldier_X = 50;
		mENEMY_0_Soldier_Y = 230;
	}
	~ENEMY_0_Soldier()
	{
		GoBitmap->ReleaseBmp(mENEMY_0_Soldier_Bmpid);
	}
	void InitPiece()                    //加载棋子
	{
		mENEMY_0_Soldier_Bmpid = GoBitmap->Load_Bmp("Image\\Piece\\Red_Square\\Soldier.bmp");
	}
	void RenderPiece()            //渲染棋子
	{
		m.Translate(mENEMY_0_Soldier_X, mENEMY_0_Soldier_Y);
		GoBitmap->SetWorldTransform(m);
		GoBitmap->DrawBmp2(mENEMY_0_Soldier_Bmpid, -30, -30, 60, 60, 0, 0, 60, 60, 0XFF00FF);
		m.Identity();
		GoBitmap->SetWorldTransform(m);
	}
	void PieceLogicUpdata(int Which, int State, int Mouse_X, int Mouse_Y)  //棋子逻辑更新
	{
		if (Piece::Reutrn_mWe() == 1)//1表示我可以走棋了
		{
			if (Which == 1 && State == 1)
			{
				if (Mouse_X >= mENEMY_0_Soldier_X - 30 && Mouse_X <= mENEMY_0_Soldier_X + 30
					&& Mouse_Y >= mENEMY_0_Soldier_Y - 30 && Mouse_Y <= mENEMY_0_Soldier_Y + 30)
				{
					if (Piece_Logic::mWE_CommandState || Piece_Logic::mWE_CommandState == 0)
					{
						Piece_Logic::mWE_CommandState = 170;//小兵被锁定
					}
				}
				if (Piece_Logic::mWE_CommandState == 170)//小兵被锁定
				{
					BOOL BoolForward = FALSE;
					BOOL BoolLeft = FALSE;
					BOOL BoolRight = FALSE;
					BoolForward = _____Elephant__TO__Soldier__Logic__::ENEMY__Soldier_MoveForwardTarget(mENEMY_0_Soldier_X, mENEMY_0_Soldier_Y, Mouse_X, Mouse_Y);
					BoolLeft = _____Elephant__TO__Soldier__Logic__::ENEMY__Soldier_MoveLeftTarget(mENEMY_0_Soldier_X, mENEMY_0_Soldier_Y, Mouse_X, Mouse_Y);
					BoolRight = _____Elephant__TO__Soldier__Logic__::ENEMY__Soldier_MoveRightTarget(mENEMY_0_Soldier_X, mENEMY_0_Soldier_Y, Mouse_X, Mouse_Y);
	...
	{
		__BoolForward(BoolForward, mENEMY_0_Soldier_Y, 170, 0);
		__BoolLeft(BoolLeft, mENEMY_0_Soldier_X, 170, 0);
		__BoolRight(BoolRight, mENEMY_0_Soldier_X, 170, 0);
		return 0;
	}
