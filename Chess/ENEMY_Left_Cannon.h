...
class ENEMY_Left_Cannon :public Piece, public Piece_Logic,public _____Elephant__TO__Soldier__Logic__,public ENEMY_Command
{
...
public:
	...
	void PieceLogicUpdata(int Which, int State, int Mouse_X, int Mouse_Y)  //棋子逻辑更新
	{
		if (Piece::Reutrn_mWe() == 1)//1表示我可以走棋了
		{
			if (Which == 1 && State == 1)
			{
				if (Mouse_X >= mENEMY_Left_Cannon_X - 30 && Mouse_X <= mENEMY_Left_Cannon_X + 30
					&& Mouse_Y >= mENEMY_Left_Cannon_Y - 30 && Mouse_Y <= mENEMY_Left_Cannon_Y + 30)
				{
					if (Piece_Logic::mWE_CommandState || Piece_Logic::mWE_CommandState == 0)
					{
						Piece_Logic::mWE_CommandState = 120;//左边的炮被锁定
					}
				}
				if (Piece_Logic::mWE_CommandState == 120)//左边的炮被锁定
				{
					BOOL BoolForward = FALSE;
					BoolForward = _____Elephant__TO__Soldier__Logic__::Cannon_MoveForwardTarget(mENEMY_Left_Cannon_X, mENEMY_Left_Cannon_Y, Mouse_X, Mouse_Y);
					BOOL BoolBack = FALSE;
					BoolBack = _____Elephant__TO__Soldier__Logic__::Cannon_MoveBackTarget(mENEMY_Left_Cannon_X, mENEMY_Left_Cannon_Y, Mouse_X, Mouse_Y);
					BOOL BoolLeft = FALSE;
					BoolLeft = _____Elephant__TO__Soldier__Logic__::Cannon_MoveLeftTarget(mENEMY_Left_Cannon_X, mENEMY_Left_Cannon_Y, Mouse_X, Mouse_Y);
					BOOL BoolRight = FALSE;
					BoolRight = _____Elephant__TO__Soldier__Logic__::Cannon_MoveRightTarget(mENEMY_Left_Cannon_X, mENEMY_Left_Cannon_Y, Mouse_X, Mouse_Y);
	...
	int __WE_Left_Cannon_Logic___(BOOL BoolForward, BOOL BoolBack, BOOL BoolLeft, BOOL BoolRight)
	{
		if (BoolForward)
		{
			for (int i = 0; i < 12; i++)
			{
				...
				{
					bool IfOkMove = false;
					if (Piece_Logic::mMouseDownRange[i][j] == 120)//索引到棋子左边的炮
					{
						...
						int Move_Y = (mENEMY_Left_Cannon_Y - Temp_Y) / 60;//得到要移动的次数，同时也是目标的Y坐标
						...
						for (int k = 0; k < Move_Y; k++)//进行巡路是否可以行走，可以走多少
						{
							...
							if ((Piece_Logic::mMouseDownRange[i - (k + 1)][j] < 200 &&
									Piece_Logic::mMouseDownRange[i - (k + 1)][j] != 999))
							{
								if (Index_Frequency == 0)//保证只执行一次
								{
									...
									if (Index == 1)//通过这个桥梁找敌方棋子
									{
										...
										{
											Index_N_I++;
											if ((Piece_Logic::mMouseDownRange[Index_I - (n + 1)][Index_J] <= 17 &&
							...
							if (Index == 2)//炮找到了攻击的对象
							{
								...
								mENEMY_Left_Cannon_Y -= (Move_Y * 60);
			...
			for (int i = 0; i < 12; i++)
			{
				...
				for (int j = 0; j < 11; j++)
				{
					...
					if (Piece_Logic::mMouseDownRange[i][j] == 120)//索引到棋子左边的炮
					{
						int Temp_X = ENEMY_Command::__mReturnRedCurStandardCoor__().x;
						int Temp_Y = ENEMY_Command::__mReturnRedCurStandardCoor__().y;//得到鼠标点击的标准坐标
						int Move_Y = (Temp_Y - mENEMY_Left_Cannon_Y) / 60;//得到要移动的次数，同时也是目标的Y坐标
						int Index = 0;//通过这个索引来执行吃棋
						int Index_I = 0;//记录索引到的第一个棋
						int Index_J = 0;//记录索引到的第一个棋
						int Index_I_J = 0;//记录索引到的第一个棋的次数
			...
			for (int i = 0; i < 12; i++)
			{
				...
				for (int j = 0; j < 11; j++)
				{
					...
					if (Piece_Logic::mMouseDownRange[i][j] == 120)//索引到棋子左边的炮
					{
						int Temp_X = ENEMY_Command::__mReturnRedCurStandardCoor__().x;
						int Temp_Y = ENEMY_Command::__mReturnRedCurStandardCoor__().y;//得到鼠标点击的标准坐标
						int Move_X = (mENEMY_Left_Cannon_X - Temp_X) / 60;//得到要移动的次数，同时也是目标的Y坐标
						int Index = 0;//通过这个索引来执行吃棋
						int Index_I = 0;//记录索引到的第一个棋
						int Index_J = 0;//记录索引到的第一个棋
						int Index_I_J = 0;//记录索引到的第一个棋的次数
						...
						{
							Index_I_J++;//记录索引到的第一个棋的次数
							if ((Piece_Logic::mMouseDownRange[i][j - (k + 1)] < 200 &&
									Piece_Logic::mMouseDownRange[i][j - (k + 1)] != 999))
							{
								if (Index_Frequency == 0)//保证只执行一次
								{
									...
									if (Index == 1)//通过这个桥梁找敌方棋子
									{
										...
										{
											Index_N_I++;
											if ((Piece_Logic::mMouseDownRange[Index_I][Index_J - (n + 1)] <= 17
							...
							if (Index == 2)//炮找到了攻击的对象
							{
								...
								Piece_Logic::mMouseDownRange[i][j] = 258;
								mENEMY_Left_Cannon_X -= (Move_X * 60);
			...
			for (int i = 0; i < 12; i++)
			{
				...
				for (int j = 0; j < 11; j++)
				{
					...
					if (Piece_Logic::mMouseDownRange[i][j] == 120)//索引到棋子左边的炮
					{
						int Temp_X = ENEMY_Command::__mReturnRedCurStandardCoor__().x;
						int Temp_Y = ENEMY_Command::__mReturnRedCurStandardCoor__().y;//得到鼠标点击的标准坐标
						int Move_X = (Temp_X - mENEMY_Left_Cannon_X) / 60;//得到要移动的次数，同时也是目标的Y坐标
						int Index = 0;//通过这个索引来执行吃棋
						int Index_I = 0;//记录索引到的第一个棋
						int Index_J = 0;//记录索引到的第一个棋
						int Index_I_J = 0;//记录索引到的第一个棋的次数
